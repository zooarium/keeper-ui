...
export function useDivisions(appId) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { invalidate(); showNotification('Division created!', 'success'); },
    onError: (err) => showNotification(err.message, 'error'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => updateDivision(id, payload),
    onSuccess: () => { invalidate(); showNotification('Division updated!', 'success'); },
    onError: (err) => showNotification(err.message, 'error'),
  });

  const removeMutation = useMutation({
    mutationFn: (id) => deleteDivision(id),
    onSuccess: () => { invalidate(); showNotification('Division deleted!', 'success'); },
    onError: (err) => showNotification(err.message, 'error'),
  });
